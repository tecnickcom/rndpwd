...
test:
	@mkdir -p target/test
	GOPATH=$(GOPATH) go test -bench=. -race -v ./... | tee >(PATH=$(GOPATH)/bin:$(PATH) go-junit-report > target/test/report.xml); test $${PIPESTATUS[0]} -eq 0
...
deb: build
	...
	mkdir -p $(PATHDEBPKG)/$(PKGNAME)-$(VERSION)/debian/missing-sources/$(BINPATH)
	echo "//" > $(PATHDEBPKG)/$(PKGNAME)-$(VERSION)/debian/missing-sources/$(BINPATH)/$(PROJECT)
